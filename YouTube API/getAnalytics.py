...
CLIENT_SECRETS_FILE = "[INSERT YOUR OWN]"
...
def getAnalytics(youtube_analytics, channel_id):
  ...
  with open (args.name+'.csv', 'w') as output:
    ...
    i = 0
    headers = ['PubDate','PubHour','Title','ID','Views','Likes','Dislikes','Shares','Comments','Minutes Watched','Average % Viewed','Subscribers Gained','Subscribers Lost','Estimated Revenue', 'CPM', 'Monetized Playbacks', 'Gross Revenue']
    writer.writerow(headers)
    while i < len(videos):
      videoid = videos[i]
      sys.stdout.write('\rCurrently getting information for: %s Progress: %i/%i' % (str(videoid), int(i)+1, int(len(videos))))
      sys.stdout.flush()
...
if __name__ == "__main__":
  ...
  argparser.add_argument("--metrics", help="Set metrics. Default is= views,likes,dislikes,shares,comments,estimatedMinutesWatched,averageViewPercentage,subscribersGained,subscribersLost", 
    default='views,likes,dislikes,shares,comments,estimatedMinutesWatched,averageViewPercentage,subscribersGained,subscribersLost,estimatedRevenue,cpm,monetizedPlaybacks,grossRevenue')
  args = argparser.parse_args()


  (youtube, youtube_analytics) = get_authenticated_services(args)
  try:
    channel_id = '[INSERT YOUR OWN]'#get_channel_id(youtube)
